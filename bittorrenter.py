...
class BittorrentProtocol(Protocol):
    ...
    def dataReceived(self,data):
       # print 'data Received:',data
        ...
        #print 'Messages to send:',str(messages_to_send)
    ...
    def parse_messages(self, messages_to_send_list):
        ...
        if isinstance(message_obj, messages.Choke):
            ...
            self.pending_requests -= 1
            percentage=self.factory.active_torrent.write_block(message_obj)
            #print percentage
            if self.interested and not self.choked:
                messages_to_send_list.append(self.get_next_request())
    ...
    def parse_message_from_response(self):   
        #classifies msgs into their types!
        length = messages.bytes_to_number(self.message_buffer[0:4]) + 4  
        #length indicated by the first 4 bytes + 4 for those first 4 bytes
        bytestring = self.message_buffer[:length]
        #print 'msg_id:',self.message_buffer[4]              #5th byte is msg-id 
